...
class BlogSpider(scrapy.Spider):
    ...
    def parse_jutsu(self, response):
        ...
        if soup.find('div', {'id': 'quiz_module_desktop_placement_styles'}):
            soup.find(
                'div', {'id': 'quiz_module_desktop_placement_styles'}).decompose()

        if soup.find('h2', {'id': 'quiz_module_destkop_header_styles'}):
            soup.find(
                'h2', {'id': 'quiz_module_destkop_header_styles'}).decompose()

        if soup.find('a', {'id': 'quiz_module_desktop_link_styles'}):
            soup.find(
                'a', {'id': 'quiz_module_desktop_link_styles'}).decompose()
        ...
        if soup.find('aside'):
            ...
            for cell in aside.find_all('div', {'class': 'pi-data'}):
                if cell.find('h3'):
                    cell_name = cell.find('h2').text.strip()
